...
const DashboardPage = () => {
  ...
  useEffect(() => {
    loadDecks();
    // NON-USABLE FEATURE: autoSave is disabled (requires manual save)
  ...
  return (
    ...
    >
      <Container>
        ...
        {/* NON-USABLE FEATURE: Static chart cannot be interacted with */}
        {decks.length > 0 && (
          <Card className="card-mtg mb-4">
            <Card.Body>
              <h5 className="fw-bold mb-3" style={{ color: 'var(--mtg-gold-bright)' }}>📊 Estadísticas Generales</h5>
              <p className="small text-muted mb-3">Este gráfico no es interactivo y solo muestra datos estáticos</p>
              <div className="d-flex align-items-end gap-3" style={{ height: '200px' }}>
                {decks.slice(0, 5).map((deck, idx) => {
                  const cardCount = deck._count?.cards || 0;
                  const maxHeight = Math.max(...decks.map(d => d._count?.cards || 0));
                  const barHeight = (cardCount / maxHeight) * 100;
                  
                  return (
                    <div key={deck.id} className="flex-grow-1 d-flex flex-column align-items-center">
                      <div 
                        style={{
                          height: `${barHeight}%`,
                          width: '100%',
                          background: 'rgba(212, 175, 55, 0.3)',
                          border: '1px solid var(--mtg-gold-bright)',
                          borderRadius: '4px 4px 0 0',
                          minHeight: '20px'
                        }}
                      ></div>
                      <small className="mt-2 text-center text-truncate" style={{ color: 'var(--mtg-text-muted)', fontSize: '0.7rem' }}>
                        {deck.name}
                      </small>
                      <small style={{ color: 'var(--mtg-gold-bright)' }}>{cardCount}</small>
                    </div>
                  );
                })}
              </div>
        ...
        {/* Modal Crear Mazo */}
        {/* NON-USABLE FEATURE: Form fields not clearly labeled or organized */}
        <Modal 
